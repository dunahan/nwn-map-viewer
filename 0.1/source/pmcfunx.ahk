...
ReadDCLConfig() {
  Global
    ...
    ;Options
    IniRead, NWN_DIR, config.ini, Main, GameDirectory, C:\NeverwinterNights\NWN
    IniRead, TRACK_INI, config.ini, Main, TrackingIni, playertrack.ini
...
CreateDCLConfig() {
  Global
    ...
    ;Options
    IniWrite, C:\NeverwinterNights\NWN, config.ini, Main, GameDirectory
    IniWrite, playertrack.ini, config.ini, Main, TrackingIni
...
;gets the token by the position within the haystack
GetTokenByPosition(haystack, position, delim="`|") {
  tokens = GetTokenCount(haystack, delim)
  result = "ERROR"
  Loop, Parse, haystack, %delim%
  {
    While A_Index < tokens
    {
      If (A_Index == position)
      {
        result := A_LoopField

...
MapBuild:
  ...
  Loop, %MapGridX%
  {
    Gui, 2: Add, Picture, x%MapX% y%PicY% +BackgroundTrans, %A_WorkingDir%\icons\MapGrid.png
    MapX := MapX+32
  }
  
  Loop, %MapGridY%
  {
    Gui, 2: Add, Picture, x%PicX% y%MapY% +BackgroundTrans, %A_WorkingDir%\icons\MapGrid.png
    MapY := MapY+32
  }
  ...
  Loop, %PlayerLoop%
  {
    ...
    If (AreaResRef == ResAreaResRefControl)
    {
      ...
      If (FirstChar < 0)
      {
        ...
        PosY := (LocX*MapGridX)-8 ; (14*4) - 8 = 48
        PosY := (MapW-PosY)       ; (128-48)   = 80
        PosX := (LocY*MapGridY)-8 ; (15*4)     = 60
        
        Gui, 2: Add, Text,      w110 h20, %PlayerName%  ; more players here?
        Gui, 2: Add, Text, x+10 y%MapPosY% h20, (%LocX%/%LocY% | %PosX%/%PosY%)
      ...
      Else
      {
        ...
        ;PosX := (LocX*MapGridX)-8 ; (14*4) - 8 = 48
        ;PosX := (MapW-PosX)       ; (128-48)   = 80
        ;PosY := (LocY*MapGridY)-8 ; (15*4)     = 60
...
Options:
  ...
  Gui, 3: Add, GroupBox, x188 y2 w290 h65, Directories
  Gui, 3: Add, Text, x202 y25 w60 h20, MiniMaps:
  Gui, 3: Add, Edit, x262 y22 w160 h20 vMAP_DIR, %MAP_DIR%
  Gui, 3: Add, Button, x425 y22 w45 h20 gBrowseMapDirectory, Browse
  Gui, 3: Add, Text, x202 y45 w60 h20, Playertrack:
  Gui, 3: Add, Edit, x262 y42 w160 h20 vTRACK_INI, %TRACK_INI%
  Gui, 3: Add, Button, x425 y42 w45 h20 gBrowseTrackIni, Browse

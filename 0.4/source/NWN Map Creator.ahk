  ...
  If (PlayerMap == 1)
  {
    ...
    Loop, Parse, Active, |,
    {
      ...
      If (AreaResRef == ResAreaResRefControl)
      {
        ...
        Gui, 2: Add, Text, x10  y%MapPosY% h20 gHighlightPlayer, %PlayerName%  (%NwnX%/%NwnY%) | (%PosX%/%PosY%)
        ; set MapPin visible
  ...
  If (CritterMap == 1)
  {
    ...
    Loop, Parse, CritterMapToken, |,
    {
      IfEqual, A_LoopField, %ResAreaResRefControl%
      {
        ...
        Gui, 2: Add, Text, x%CritX%  y%MapPosY% h20, %Found%
  ...
  If A_GuiEvent = DoubleClick
  {
    ...
    Loop, Parse, Active, |,
    {
      ...
      If (ResMapPlayerControl == PlayerName)
      {
        ...
        ;If (DEBUG == 1)
        ;  ResultMapLoop = %PlayerName% (%NwnX%/%NwnY%)
...
Options:
  ...
  If (ExpertsMode == 1)
  {
    Gui, 3: Add, GroupBox, x188 y2 w290 h145, Directories
